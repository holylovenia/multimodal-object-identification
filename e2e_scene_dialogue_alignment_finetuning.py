...
def run(model_args, data_args, training_args):
    training_args.output_dir="{}/{}|{}|_{}_lr{}_bs{}".format(
        ...
        model_args.vision_model_name_or_path.replace("/", "__"),
        model_args.text_model_name_or_path.replace("/", "__"),
    ...
    if model_args.text_model_name_or_path == "gpt2":
    ...
    train_loader = data_utils.Dataloader(tokenizer, data_args.train_dataset_path, data_args)
    dev_loader = data_utils.Dataloader(tokenizer, data_args.dev_dataset_path, data_args)
    devtest_loader = data_utils.Dataloader(tokenizer, data_args.devtest_dataset_path, data_args)
    ...
    print()
    print("=== dev")
    for i in range(dev_loader.num_instances):
        batch = dev_loader.get_indexed_data([i])
    print()
    print("=== devtest")
    ...
    print(dev_loader.get_indexed_data([5]))
    quit()

    # Model.
    model = AmbiguousCandidateIdentifier(tokenizer, model_args)
    model.train()
    # Loss function.
    criterion = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([model_args.positive_weight]))
    criterion = criterion.cuda()
    # Prepare optimizer and schedule (linear warmup and decay).
    optimizer = transformers.AdamW(
        model.parameters(), lr=training_args.learning_rate, eps=training_args.adam_epsilon
    )

    total_steps = (
        int(train_loader.num_instances / training_args.per_device_train_batch_size * training_args.num_train_epochs) + 1
    )
    num_iters_epoch = train_loader.num_instances // training_args.per_device_train_batch_size
    num_iters_epoch_float = train_loader.num_instances / training_args.per_device_train_batch_size
    next_eval_iter = 0
    num_iters = 0
